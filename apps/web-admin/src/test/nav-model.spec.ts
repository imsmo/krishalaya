...
describe('admin nav model', () => {
  it('live routes are exactly the built ones (grows as waves land)', () => {
    expect(liveNav().map((i) => i.href)).toEqual(['/dashboard', '/ai-models', '/tenants', '/reports', '/flags', '/recon', '/billing', '/plans', '/providers', '/support', '/compliance', '/impersonation', '/staff', '/staff/me', '/analytics/reports', '/analytics/exports', '/templates', '/settings', '/announcements', '/catalogue', '/schemes-registry', '/cells', '/moderation']);
  });
