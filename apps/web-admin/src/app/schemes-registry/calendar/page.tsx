...
import { closeTone, closeKey, type CloseState } from '../../../features/schemes-registry/version';
  ...
  const cols: Column<CalendarRow>[] = [
    ...
    // or impossible window DOES read as a failure: both mean a stored date nobody can act on.
    {
      ...
      cell: (r) => {
        ...
        return <StatusPill tone={closeTone(cs)} label={key === 'closesIn' && cs.kind === 'closes_in' ? t.t('sv.close.closesIn', { days: String(cs.days) }) : t.t(`sv.close.${key}`)} />;
      },
    },
  ];
  const nextHref = () => {
  ...
  return (
    <section>
      ...
      <form method="get" className="kv-form kv-filters" aria-label={t.t('sr.calendarFilter')}>
        ...
        <Button type="submit">{t.t('sr.calendarApply')}</Button>
      </form>
      ...
      {notice ? <p className="kv-error" role="alert">{notice}</p> : (
        <>
          ...
          {closingSoon.length === 0
            ? <EmptyState title={t.t('sv.closingSoonEmpty')} />
            : (
              <ul>
                {closingSoon.map((c) => (
                  <li key={c.id}>
                    <Link href={`/schemes-registry/schemes/${encodeURIComponent(c.id)}`}>{c.code}</Link> — {c.defaultName}{' '}
                    <StatusPill tone={closeTone(c.closeState)} label={c.closeState.kind === 'closes_in' ? t.t('sv.close.closesIn', { days: String(c.closeState.days) }) : t.t(`sv.close.${closeKey(c.closeState)}`)} />
                  </li>
                ))}
              </ul>
            )}
          ...
          {!nudge.available && (
            <>
              ...
              <Callout>{t.t('sv.nudgeNotBuilt')}</Callout>
