...
export class MaintenanceRepository {
  ...
  async listLogs(tenantId: string, assetId: string): Promise<Array<Record<string, unknown>>> {
    ...
    return r.rows.map((x: any) => ({ id: x.id, logType: x.log_type, costMinor: x.cost_minor, notes: x.notes, engineHoursAt: x.engine_hours_at, performedOn: String(x.performed_on).slice(0, 10) }));
  }
  async alerts(tenantId: string): Promise<Array<Record<string, unknown>>> {
    ...
    return r.rows.map((x: any) => ({ assetId: x.asset_id, status: x.status, latestLog: x.latest_log, latestOn: x.latest_on ? String(x.latest_on).slice(0, 10) : null, lastServiceOn: x.last_service_on ? String(x.last_service_on).slice(0, 10) : null, alert: x.alert }));
  }
}
