...
describe('DocumentPdfService — Q23 billing badge (pure render, structural assertions)', () => {
  it('renderStatement: tenant brand name + platform badge appear in the header, before the body', () => {
    ...
    const bytes = svc.renderStatement(STATEMENT, { tenantName: 'Anand FPO', badgeText: 'Powered by Krishalaya' });
    const text = bytes.toString('latin1');
    expect(text).toContain('Anand FPO');
    expect(text).toContain('Powered by Krishalaya');
    ...
    expect(text.indexOf('Powered by Krishalaya')).toBeLessThan(text.indexOf('Seller: seller-1'));
  ...
  it('renderInvoice: same header law applied to trade invoices', () => {
    const { svc } = makeService();
    const bytes = svc.renderInvoice(INVOICE, { tenantName: 'Anand FPO', badgeText: 'Powered by Krishalaya' });
    const text = bytes.toString('latin1');
    expect(text).toContain('Anand FPO');
    expect(text).toContain('Powered by Krishalaya');
    expect(text.indexOf('Powered by Krishalaya')).toBeLessThan(text.indexOf('Order: order-1'));
  });

  it('honest degrade (Law 12): unknown tenant name never fabricates a placeholder — badge still renders alone', () => {
    const { svc } = makeService();
    const bytes = svc.renderStatement(STATEMENT, { tenantName: null, badgeText: 'Powered by Krishalaya' });
    const text = bytes.toString('latin1');
    expect(text).toContain('Powered by Krishalaya');
  ...
  it('badge value is never one without the other: brandHeaderLines never omits the platform badge, with or without a tenant name', () => {
    const { svc } = makeService();
    const withName = svc.renderStatement(STATEMENT, { tenantName: 'Anand FPO', badgeText: 'Powered by Krishalaya' }).toString('latin1');
    const withoutName = svc.renderStatement(STATEMENT, { tenantName: null, badgeText: 'Powered by Krishalaya' }).toString('latin1');
    expect(withName).toContain('Powered by Krishalaya');
    expect(withoutName).toContain('Powered by Krishalaya');
  });
  ...
  describe('i18n key resolution (Law 3) — doc.poweredByKrishalaya across en/hi/gu', () => {
    it('resolves distinct, real strings per language from the actual bundles (not the literal key)', () => {
      const i18n = new TranslationService();
      const en = i18n.t('doc.poweredByKrishalaya', 'en');
      const hi = i18n.t('doc.poweredByKrishalaya', 'hi');
      const gu = i18n.t('doc.poweredByKrishalaya', 'gu');
      expect(en).toBe('Powered by Krishalaya');
      expect(hi).not.toBe('doc.poweredByKrishalaya');
      expect(gu).not.toBe('doc.poweredByKrishalaya');
      expect(hi).not.toBe(en);
      expect(gu).not.toBe(en);
      expect(hi).toContain('Krishalaya');   // brand name kept in Latin script, sms.otp convention
      expect(gu).toContain('Krishalaya');
    });

    it('an unregistered locale falls back to English (TranslationService\'s own degrade law)', () => {
      const i18n = new TranslationService();
      expect(i18n.t('doc.poweredByKrishalaya', 'fr')).toBe('Powered by Krishalaya');
    });
    ...
    it('this renderer\'s own WinAnsi-only constraint: hi/gu badge TEXT actually printed into the PDF stays', () => {
      ...
      const { svc } = makeService();
      const bytesHi = svc.renderStatement(STATEMENT, { tenantName: null, badgeText: 'Powered by Krishalaya' });
      ...
      expect(bytesHi.toString('latin1')).toContain('Powered by Krishalaya');
    });
    ...
    it('storeStatementPdf(lang="hi"): resolves through TranslationService but the PDF byte stream carries the WinAnsi-safe EN fallback, never "?" mojibake', async () => {
      ...
      const pdfArg: Buffer = media.putGeneratedDocument.mock.calls[0][1];
      const text = pdfArg.toString('latin1');
      expect(text).toContain('Powered by Krishalaya');   // ASCII-safe fallback, not garbled Devanagari
