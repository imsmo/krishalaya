...
run('PC-56 TENANT-6c-1 · the dairy payout cycle (integration, real Postgres)', () => {
  ...
  describe('the cycle closes and its bills get built — the thing that has never happened on this platform', () => {
    ...
    it('REFUSES to pay a bill carrying deductions — nowhere to post them, so nothing moves', async () => {
      // Generated by hand for a period of its own, because the cycle path never invents a deduction.
      const day2 = win.from;
      await collections.record(tenantIST, actor, `idem-${randomUUID()}`, {
      ...
      const withDeduction = await bills.generate(tenantIST, actor, `idem-${randomUUID()}`,
        { membershipId: memB, periodStart: day2, periodEnd: day2, deductions: [{ type: 'loan_emi', amountMinor: '30000' }] } as never);
      await bills.preview(tenantIST, actor, withDeduction.id);
      ...
      expect(still.rows[0].status).toBe('approved');  // and it is NOT marked paid
      // Before this wave it WOULD have paid: the net moved, the Rs 300 "loan EMI" was posted to no account, and the
      // family's loan was untouched — so they paid that instalment twice.
    });
  });
