...
/**
 * [PC-56 TENANT-6c-4] ONE deduction line, as the bill sees it.
 *
 * `type` used to be a free-typed 40-character string in a jsonb blob (0009) referencing nothing; it is now a
 * `lookup_values` code (type `milk_deduction`) with the row it PAYS beside it. `id` is the `milk_bill_deductions`
 * row — the same line the applier stamps and the ledger txn points back at.
 */
...
export interface MilkBillProps {
  ...
  grossMinor: bigint;
  /**
   * [PC-56 TENANT-6c-4] The lines, or NULL when this bill was read WITHOUT them.
   *
   * `[]` and "not loaded" are different facts and the difference is a member's money, so list reads (which do not
   * join 312 bills to their lines on a partitioned money table) carry `null` and every reader of the lines gets a
   * refusal instead of a silent zero. `deductionsMinor` below is on the bill row and is always present.
   */
  deductions: BillDeduction[] | null;
...
export class MilkBill {
  ...
  static generate(input: { id: string; tenantId: string; membershipId: string; cycleId?: string | null; periodStart: string; periodEnd: string;
    ...
    // The bill's total is DERIVED from its lines, every time, and never accepted from a caller: a gross, a set of
    // lines and a total that disagree is a bill whose net nobody can defend.
  ...
  get grossMinor() { return this.props.grossMinor; }
  get deductionsMinor() { return this.props.deductionsMinor; }
  /** The lines, or a refusal if this bill was read without them. */
  get deductionLines(): BillDeduction[] {
    if (this.props.deductions === null) throw new DeductionLinesNotLoadedError(this.props.id);
    return this.props.deductions;
  }
  get hasLoadedDeductions(): boolean { return this.props.deductions !== null; }
  ...
  toJSON() { const v = this.props; return { id: v.id, membershipId: v.membershipId, cycleId: v.cycleId, periodStart: v.periodStart, periodEnd: v.periodEnd,
    ...
    // Lines are present when they were LOADED and absent otherwise — never `[]` standing in for "we did not look".
    ...(v.deductions === null ? {} : { deductions: v.deductions.map((d) => ({ id: d.id, type: d.type, amountMinor: d.amountMinor.toString(), sourceType: d.sourceType, sourceId: d.sourceId, status: d.status })) }),
    deductionsMinor: v.deductionsMinor.toString(),
