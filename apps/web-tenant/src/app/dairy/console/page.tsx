// apps/web-tenant/src/app/dairy/console/page.tsx · THE PRE-CANON DAIRY OPERATOR CONSOLE, moved here intact by
// PC-56 TENANT-6a.
//
// This page was built as P1-12: one screen with five sections (MCCs, rate cards, members, counter collection-entry,
// and the milk-bill settlement workflow). The canon splits that work across SIX screens — W167 Collections,
// W168 Quality, W169 Payout cycles, W170 BMC, W171 Centres, W172 Insights — and `/dairy` is W167's, so this page moved
// rather than being deleted: an FPO that uses it today to enrol a member or approve a bill would otherwise lose that
// ability the moment the canon's first screen shipped. It is linked from the dairy sub-nav as `legacy` and will empty
// out as TENANT-6b–6e give each of its sections a canon home; the last wave to take a section deletes the page.
//
// Nothing inside it was changed by this wave except the import depths and the component name — deliberately, because
// a "move" that also edits behaviour is two changes wearing one commit.
...
export default async function DairyOperatorConsolePage({ searchParams }: { searchParams: { ok?: string; error?: string } }) {
