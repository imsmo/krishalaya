// apps/web-tenant/src/app/dairy/insights/exports/[id]/page.tsx · W2553 (Export queued) + W2554 (Export ready) — PC-56 TENANT-6e-2.
...
import {
  byteParts, byteUnitKey, downloadErrorKey, downloadStateKey, etaKey, etaParts, etaUnitKey, exportHref, exportState,
  exportStateKey, exportTitleKey, exportTransportState, failureKeyFor, shaGroups,
} from '../../../../../features/dairy/exports';
import { INSIGHTS_HREF } from '../../../../../features/dairy/insights';
...
export default async function DairyExportPage({ params, searchParams }: { params: { id: string }; searchParams: { error?: string } }) {
  const id = params.id;
  ...
  return (
    <section>
      <nav aria-label={t.t('dairy.export.breadcrumb')} className="kv-field__hint">
        <Link href={INSIGHTS_HREF}>{t.t('dairy.insights.title')}</Link>{' / '}{t.t(exportTitleKey(state))}
      </nav>
      ...
      {!job && (
        <div className={state === 'error' ? 'kv-error' : 'kv-card kv-card--notice'} role={state === 'error' ? 'alert' : 'status'}>
          ...
          <p><Link href={INSIGHTS_HREF} className="kv-btn kv-btn--secondary">{t.t('dairy.export.back')}</Link></p>
        </div>
      )}
      ...
      {job && (state === 'queued' || state === 'running') && (
        <div className="kv-card" role="status" aria-live="polite">
          ...
          <p>
            ...
            <Link href={INSIGHTS_HREF} className="kv-btn kv-btn--secondary">{t.t('dairy.export.back')}</Link>
          </p>
        </div>
      )}
      ...
      {job && state === 'failed' && (
        <div className="kv-error" role="alert">
          ...
          <p><Link href={INSIGHTS_HREF} className="kv-btn kv-btn--secondary">{t.t('dairy.export.backToScreen')}</Link></p>
        </div>
      )}
      ...
      {job && receipt && (state === 'ready' || state === 'expired') && (
        <>
          ...
          <p><Link href={INSIGHTS_HREF} className="kv-btn kv-btn--primary">{t.t('dairy.export.backToScreen')}</Link></p>
        </>
      )}
    </section>
  );
