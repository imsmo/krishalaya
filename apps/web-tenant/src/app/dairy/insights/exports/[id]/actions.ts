'use server';
// apps/web-tenant/src/app/dairy/insights/exports/[id]/actions.ts · W2554's *"Download (link valid 15 min)"* (PC-56 TENANT-6e-2).
//
...
import { exportDownloadHref, exportHref } from '../../../../../features/dairy/exports';

export async function mintDownloadLinkAction(formData: FormData): Promise<void> {
  const id = String(formData.get('id') ?? '').trim();
  if (!id) redirect('/dairy/insights');
