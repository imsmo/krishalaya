// apps/web-ops/src/lib/api-client.ts · the tenant SDK, wired with the console's session token (read
...
export function opsClient(): KrishalayaClient {
  return createClient({
    ...
    userAgent: 'kv-web-ops',
    timeoutMs: 8000,
  });
}
export function anonClient(): KrishalayaClient {
  return createClient({ baseUrl: env.serverApiUrl, userAgent: 'kv-web-ops', timeoutMs: 8000 });
}
