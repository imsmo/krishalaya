// apps/mobile/src/features/fpo-coordinator/screens/MemberPledgesScreen.tsx · screen · [P2]
// DEV-12 (2026-07-24, Group-B EmptyState pass set 2): canon ref `screens/374-farmer-contribution-entry.html`
// ("374 · Contribution Entry" — nearest concept — no literal member-pledges screen exists; nearest existing group-contribution pattern). Behind `fpo_coordinator` (DEV-08 census §2 Group-B genuine gap — zero
...
export default function MemberPledgesScreen() {
  const { t } = useTranslation();
  const enabled = useFlag('fpo_coordinator');
  const state = offModuleState(enabled);
  return (
    <ScreenScaffold title={t('fpoCoordinatorMemberPledges.title')}>
      <EmptyState
        ...
        testID="fpo-member-pledges-empty-state"
      />
    </ScreenScaffold>
  );
}
