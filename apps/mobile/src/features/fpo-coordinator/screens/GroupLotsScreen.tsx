// apps/mobile/src/features/fpo-coordinator/screens/GroupLotsScreen.tsx · screen · [P2]
// DEV-12 (2026-07-24, Group-B EmptyState pass set 2): canon ref `screens/371-farmer-group-detail.html`
// ("371 · Group Detail" — nearest concept — no literal group-lots-list screen exists; nearest existing group-detail pattern). Behind `fpo_coordinator` (DEV-08 census §2 Group-B genuine gap — zero
...
export default function GroupLotsScreen() {
  const { t } = useTranslation();
  const enabled = useFlag('fpo_coordinator');
  const state = offModuleState(enabled);
  return (
    <ScreenScaffold title={t('fpoCoordinatorGroupLots.title')}>
      <EmptyState
        ...
        testID="fpo-group-lots-empty-state"
      />
    </ScreenScaffold>
  );
}
