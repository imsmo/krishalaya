// apps/web-gov/src/app/api/lang/route.ts · the locale switcher's no-JS endpoint. A plain <form method=post>
// posts `lang` (+ a `from` return path); we validate the language against the supported set (fail-closed to the
// default — never trust the client value), persist it in the `kvg_lang` cookie, and 303 back to where the user
