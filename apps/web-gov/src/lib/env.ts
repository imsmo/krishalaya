// apps/web-gov/src/lib/env.ts · the ONLY env reader. The tenant console is an AUTHENTICATED app — still no
// secrets in the browser bundle (only NEXT_PUBLIC_* reaches the client). Fails closed if the API origin is unset.
const publicApiUrl = process.env.NEXT_PUBLIC_API_URL;
if (!publicApiUrl) throw new Error('web-gov: NEXT_PUBLIC_API_URL is required');
